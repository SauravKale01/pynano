...
async def welcome(_, message):
    for user in message.new_chat_members:
        try:
            ...
            draw.ellipse((0, 0, profile_pic.size[0], profile_pic.size[1]), fill=255)
            
            # Apply the circular mask to the profile picture
            profile_pic = Image.composite(profile_pic, Image.new("RGB", profile_pic.size, "white"), mask)
            ...
            welcome_with_profile_pic.paste(profile_pic, profile_pic_position, mask)
            
            # Create a new image with the combined welcome image and profile picture
            welcome_image_with_details = Image.new("RGB", (image_width, image_height + 60))
            welcome_image_with_details.paste(welcome_with_profile_pic, (0, 0))
            
            # Add user name and join group name to the image
            ...
            welcome_image_with_details.save(welcome_image_path)
